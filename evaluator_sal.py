...
from src.singlecropdataset import EvalSALDataset
...
def get_dataset(args, mode, threshold=None, match=None):
    ...
    dataset = EvalSALDataset(predict_path, gt_path, threshold=threshold, match=match, mode=mode, label_path = args.predict_path)
    dataset.set_attrs(
        batch_size=1,
        shuffle=False,
...
def match(args, mode, num_classes, threshold):
    ...
    for target, _, predict, _, logit, sal, lab in tqdm(loader):
        target = target.numpy()
        # print('target')
        # print(Counter(target.reshape(-1).tolist()))
...
def evaluator(args, mode, num_classes, thresholds, matches):
    ...
    for t in thresholds:
        ...
        for target, boundary_target, predict, boundary_predict, _, _, _ in tqdm(loader):
            target = target.numpy()
            predict = predict.numpy()
    ...
    print("best_mIoU: {}".format(best_mIoU))
...
def evaludation(args, mode):
    ...
    if args.curve:
        thresholds = [
            threshold / 100.0 for threshold in range(args.min, args.max + 1, 1)
        ]
    else:

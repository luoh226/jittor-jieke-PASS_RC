...
def GetFT(img3f):
    ...
    for y in range(height):
        for x in range(width):
            sal[y, x] = float(
                sqr(colorM[0] - tImg[y, x, 0]) + sqr(colorM[1] - tImg[y, x, 1]) + sqr(colorM[2] - tImg[y, x, 2]))
...
def Quantize(img3f, ratio=0.95, colorNums=(12, 12, 12)):
    ...
    for y in height_range:
        for x in width_range:
            idx1i[y, x] = int(img3f[y, x, 0] * clrTmp[0]) * w[0] + int(img3f[y, x, 1] * clrTmp[1]) * w[1] + int(
                img3f[y, x, 2] * clrTmp[2])

...
def inference_rc_map(img3i, thresh=0.75):
    img3f = img3i.astype(np.float32)
    img3f *= 1. / 255
    sal = SaliencyRC.GetHC(img3f)
    sal[np.where(sal < thresh)] = 0
    sal = sal * 255
    sal = sal.astype(np.int16)
    cv2.imwrite("sal.jpg", sal)


def test_rc_map():
    # img3i = cv2.imread("test.jpg")
    img3i = cv2.imread(
        "/home/share/luohan/jittor/PASS/data/ImageNetS/ImageNetS50/validation/n12998815/ILSVRC2012_val_00005745.JPEG")
    img3f = img3i.astype(np.float32)
    img3f *= 1. / 255

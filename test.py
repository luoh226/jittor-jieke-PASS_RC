...
def parse_args():
    ...
    parser.add_argument('--match_file',
                        type=str,
                        default='weights/test/pixel_finetuning/match.json',
                        help='The matching file for test set.')
    parser.add_argument('--data_path',
                        type=str,
                        default='data/ImageNetS/ImageNetS50',
                        help='The path to ImagenetS dataset.')
    parser.add_argument('--pretrained',
                        type=str,
                        default='weights/test/pixel_finetuning/checkpoint.pth.tar',
...
def main_worker(args):
    ...
    # dataset2 = InferImageFolder(root=data_path + "_last",
    #                            transform=transforms.Compose([
    #                                transforms.Resize(256),
    #                                transforms.ToTensor(),
    #                                normalize,
    ...
    dump_path = os.path.join(args.dump_path, 'result')

    targets = []
    predictions = []
    for images, path, height, width, _ in tqdm(dataloader):
        path = path[0]
        cate = path.split('/')[-2]
